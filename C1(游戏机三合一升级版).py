    ...
    while wall != None:
        while ball[1]<4:
            ...
            if button_a.is_pressed() and board>0:
                if ball[1] == 3 and (ball[0] in (board,board+1)):
                    vx -= 1
                    vy = -vy
                display.set_pixel(board-1,4,4)
                ...
                board += 1
            if ball[1]==3 and (ball[0] in (board,board+1))and vy == -1:
                vy = 1
            if vx < -1:vx=-1
            if vx > 1:vx=1
        ...
        if button_a.is_pressed():
            ...
            if res == False:
                ...
                if pin_logo.is_touched():
                    music.play(music.NYAN,loop=True,wait=False)
                    sleep(2000)
                    res,_ = greedy_snake(backup)
                    if res == False:
                        music.play(music.FUNERAL,loop=False,wait=False)
                        display.scroll('Game Over',loop=False,wait=True)
                        break
                    else:
                        break
                else:
            ...
            sleep(400)
            display.clear()
            res, backup = beatbricks([(0,0),(0,1),(1,0),(1,1),(2,0),(2,1),(3,0),(3,1),(4,0),(4,1)])
            if res == False:
                music.play(music.FUNERAL,loop=True,wait=False)
                display.scroll('Retry?',wait=True,loop=False)
                if pin_logo.is_touched():
